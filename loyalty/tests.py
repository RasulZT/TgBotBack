...
@pytest.mark.django_db
def do_payloads(order: dict[str, any], payload, action_data, action):
    """
    action_data = {
        ...
        for product in products:
            ...
            if product_id in all_data:
                ...
                for new_product_to_add in product_copy:
                    is_similar = False
                    for prod in order['products']:
                        if prod['price'] / prod['amount'] == new_product_to_add['price'] / new_product_to_add['amount'] \
    ...
    # if actual_product_ids is None:
    #     raise Exception("Что-то не так. Этого жб не может быть")


def sort_actions_by_product_lists(actions):
    """
...
def test_check_actions_logic(test_data):
    order_data = test_data['order']
    order_data_copy = copy.deepcopy(order_data)
    actions = test_data['actions']
    expected_result_order = test_data['resultOrder']
    order_company_id = order_data["company_id"]

    sorted_actions = sort_actions_by_product_lists(actions)
    ...
    for action_index, action in enumerate(sorted_actions):
        ...
        can_be_repeated = action.get("can_be_repeated", False)
        pass_checkers = True
        triggers = action['triggers']
        ...
        do_payloads(order_data, payload, action_data, action)

    print(f"Expected results: {json.dumps(expected_result_order, ensure_ascii=False)} \n"
          f"Actual result: {json.dumps(order_data, ensure_ascii=False)}")
    assert expected_result_order == order_data
